       ...
       <div class="title-box">
        ...
        <div>
            <el-table :data="tableData" style="width: 100%" stripe  @sort-change="handleSortChange">
                ...
                <el-pagination background @current-change="handleCurrentChange" :current-page="currentPage" :total="total" layout="total, prev, pager, next, jumper">
		        </el-pagination>
        </div>
...
export default {
  ...
  methods: {
    ...
    routeChange(query) {
      ...
      //发送请求这里
       this.getlist(arg)
      console.log('route change', arg)

    },
    getlist(arg){
      this.$get(`/api/queryList?`,arg)
        .then((response) => {
          this.tableData = response.vos
